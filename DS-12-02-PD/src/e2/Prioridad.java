...
public class Prioridad implements Ordenacion {
        ...
        while (!padres.isEmpty()) {
            for (int i = 0; i < hijos.size(); i++) {
                for (int j = 0; j < hijos.size(); j++) {
                    if (padres.get(i).equals(hijos.get(j))) {
                        cont = 1;
                    }
                }
                if (cont == 0) {
                    if (!aux.contains(padres.get(i)))
                        aux.add(padres.get(i));
                }
            }
            System.out.println(aux);
            Collections.sort(aux);
                for (int n = 0; n < padres.size(); n++) {
                    if (aux.contains(padres.get(n))) {
                        padres.remove(n);
                        hijos.remove(n);
                    }
            }
